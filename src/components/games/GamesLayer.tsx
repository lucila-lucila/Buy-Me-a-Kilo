...
export default function GamesLayer({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    layer.querySelector<HTMLElement>('.games__close')?.focus()
          ...
          {GAMES.map((g) => (
            <li key={g.id}>
              <button
                ...
                type="button"
                disabled={!g.ready}
                /* Los juegos llegan en las partes 4 y 5. Hasta entonces la
                   tarjeta se ve, dice la verdad y no se puede tocar: prometer
                   un juego que no está sería exactamente lo que la página no
                   hace. */
